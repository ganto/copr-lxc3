...
BuildRequires: golang(github.com/CanonicalLtd/dqlite)
BuildRequires: golang(github.com/CanonicalLtd/go-grpc-sql)
BuildRequires: golang(github.com/CanonicalLtd/go-sqlite3)
...
%if ! 0%{?with_bundled}
Requires:      golang(github.com/CanonicalLtd/go-grpc-sql)
Requires:      golang(github.com/CanonicalLtd/raft-test)
...
Requires:      golang(gopkg.in/lxc/go-lxc.v2)
...
# clear flags to avoid "flag provided but not defined: -Wl,-z,relro" error
unset LDFLAGS
...
install -p -m 755 bin/lxd %{buildroot}%{_libexecdir}/%{name}/lxd
...
install -p -m 644 scripts/bash/lxd-client %{buildroot}%{_datadir}/bash-completion/completions/lxd-client
...
install -p -m 755 %{SOURCE8} %{buildroot}%{_bindir}/lxd

# install custom libsqlite3
install -d %{buildroot}%{_libdir}/%{name}
install -p -m 755 dist/sqlite/.libs/libsqlite3.so.0* %{buildroot}%{_libdir}/%{name}
...
for file in $(find client fuidshift lxc lxd lxd-benchmark shared -iname "*.go" \! -iname "*_test.go") ; do
...
for file in $(find client fuidshift lxc lxd lxd-benchmark shared test -iname "*_test.go" -o -type f -wholename "test/deps/s*"); do
    echo "%%dir %%{gopath}/src/%%{import_path}/$(dirname $file)" >> devel.file-list
    install -d -p %{buildroot}/%{gopath}/src/%{import_path}/$(dirname $file)
    cp -pav $file %{buildroot}/%{gopath}/src/%{import_path}/$file
